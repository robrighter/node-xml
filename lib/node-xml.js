...
XMLP.prototype._parseNamespaces = function (atts){
   //return an array of objects with "prefix","namespace-URI", "scope-tag"
    // Add them to: this.m_namespaceList
}

XMLP.prototype._removeExpiredNamesapces = function (closingtagname) {
    //remove the expiring namespaces from the list (you can id them by scope-tag)
}
...
SaxParser.prototype._parseLoop = function(parser) {
    ...
    while(!this.m_bErr) {
        ...
        if(iEvent == XMLP._ELM_B) {
            ...
            this._fireEvent(SaxParser.ELM_B, nameobject.name, this.m_parser.m_atts.map(function(item){return { name : item[0], value : item[1], };}), nameobject.prefix );
        }
