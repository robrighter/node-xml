...
var parseHtmlFile = function(filename, callback){
    var posix = require('posix');
    //var that = this;
    posix.cat(filename).addCallback(function (content) {
      parseHtmlString(content,callback);
    });
}

var parseHtmlString = function(thehtml, callback){
    
   var m_doc = new HtmlDocument();
   //make a sax parser and hook up the events    
   var m_parser = new SaxParser(function(cb) {
     ...
     cb.onStartElementNS(function(elem, attrs, prefix, uri, namespaces) {
         m_doc._pushElement(elem, attrs, prefix, uri, namespaces);
     });
     cb.onEndElementNS(function(elem, prefix, uri) {
         m_doc._popElement(elem, prefix, uri);
     });
     cb.onCharacters(function(chars) {
         m_doc._pushCharactersOnCurrentElement(chars);
     });
...
HtmlDocument.prototype._pushElement = function(elem, attrs, prefix, uri, namespaces) {
    sys.puts('GOT HERE 2');
    this.m_elementstack.push(elem);
    this.m_list.push( new {
       elempath : '/' + m_elementstack.join('/'),
       attributes : attrs,
       prefix : prefix,
       uri : uri, 
    });
...
HtmlDocument.prototype._popElement = function(elem, prefix, uri){
    this.m_elementstack.pop();
}

HtmlDocument.prototype._pushCharactersOnCurrentElement = function (characters){
    sys.puts('GOT HERE 3 :' + characters);
    this.m_elementstack.push('_CHARS');
    this.m_list.push({
       elempath : '/' + m_elementstack.join('/'),
       attributes : [],
       prefix : '',
...
HtmlDocument.prototype.asString = function (){
    var toreturn = '';
    this.m_list.forEach(function (item){
        toreturn+= item.elempath + "\n";
    });
    return toreturn;
}

HtmlDocument.prototype.find = function (xpath){
    //TODO: implement xpath querys
}
