...
XMLP.prototype._parseNamespaces = function (atts){
   ...
   //The function should return a new list of tag attributes with the namespaces filtered
    ...
    //sys.puts("DEBUG: Started with atts of " + JSON.stringify(atts));
    //sys.puts("DEBUG: m_namespaceList" + JSON.stringify(this.m_namespaceList));
    //sys.puts("DEBUG: returning" + JSON.stringify(filteredatts));
    //sys.puts("DEBUG:");
    //sys.puts("DEBUG:");
    //sys.puts("DEBUG:");
    return filteredatts;
...
XMLP.prototype._getContextualNamespaces = function (prefix){
    var toreturn = [];
    //sys.puts('DEBUG: looking for namespace with prefix context of:  ' + prefix);
    this.m_namespaceList.map(function (item){
        //sys.puts('DEBUG: testing against item: ' + JSON.stringify(item));
        if((item.prefix === prefix) || (item.prefix === '')){
            toreturn.push(item.uri);
        }
    });
    
    //sys.puts("DEBUG: returning: "+JSON.stringify(toreturn));
    return toreturn;
}
...
SaxParser.prototype._fireEvent = function(iEvt) {
    ...
    if(SaxParser.DOC_B == iEvt) {
        ...
        sys.puts('DEBUG: ' + args.length + " args");
        sys.puts('DEBUG: ' + JSON.stringify(args) );
        sys.puts('DEBUG: ');
        sys.puts('DEBUG: ');
    }
...
SaxParser.prototype._parseLoop = function(parser) {
    ...
    while(!this.m_bErr) {
        ...
        if(iEvent == XMLP._ELM_B) {
            ...
            theatts = parser._parseNamespaces(theatts);
            this._fireEvent(SaxParser.ELM_B, nameobject.name, theatts, nameobject.prefix, parser._getContextualNamespaces(nameobject.prefix) );
        }
        else if(iEvent == XMLP._ELM_E) {
            nameobject = parser._parsePrefixAndElementName(parser.getName());
            parser._removeExpiredNamesapces(parser.getName());
            this._fireEvent(SaxParser.ELM_E, nameobject.name, nameobject.prefix);
        }
